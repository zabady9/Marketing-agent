...
import type { ChatMessageAdminResponse } from '../../../adminTypes'
import type { AdminFormProps } from '../AdminEntityPage'

// No create endpoint for ChatMessage — reached only via a "View messages" row
// action on the chat-sessions list — this form is edit-only in practice.
export function ChatMessageForm({ initial, onSubmit, onCancel }: AdminFormProps<ChatMessageAdminResponse>) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await onSubmit({
        content,
        role,
        tool_name: toolName.trim() === '' ? null : toolName,
        study_id: studyId.trim() === '' ? null : studyId,
      })
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-gray-700">Study ID</label>
        <input
          type="text"
          value={studyId ?? ''}
